...
/* ESPIRiT operator. */
static const struct linop_s* linop_espirit_create(long sx, long sy, long sz, long nc, long md, long ne, complex float* maps)
{
...
/* Resize operator. */
static const struct linop_s* Xlinop_reshape_create(long wx, long sx, long sy, long sz, long nc, long ne)
{
...
/* Fx operator. */
static const struct linop_s* linop_fx_create(long wx, long sy, long sz, long nc, long ne)
{
...
/* Wave operator. */
static const struct linop_s* linop_wave_create(long wx, long sy, long sz, long nc, long ne, complex float* psf)
{
	long dims[] = { [0 ... DIMS - 1] = 1};
	dims[0] = wx;
	dims[1] = sy;
...
/* Fyz operator. */
static const struct linop_s* linop_fyz_create(long wx, long sy, long sz, long nc, long ne)
{
	long dims[] = { [0 ... DIMS - 1] = 1};
	dims[0] = wx;
	dims[1] = sy;
...
/* Sampling operator. */
static const struct linop_s* linop_samp_create(long wx, long sy, long sz, long nc, long ne, complex float* mask)
{
	long dims[] = { [0 ... DIMS - 1] = 1};
	dims[0] = wx;
	dims[1] = sy;
...
int main_wave(int argc, char* argv[argc])
{
	...
	t1 = timestamp();
	const struct linop_s* E = linop_espirit_create(sx, sy, sz, nc, md, ne, maps);
	t2 = timestamp();
	debug_printf(DP_INFO, "\tE:   %f seconds.\n", t2 - t1);

	t1 = timestamp();
	const struct linop_s* R = Xlinop_reshape_create(wx, sx, sy, sz, nc, ne);
	t2 = timestamp();
	debug_printf(DP_INFO, "\tR:   %f seconds.\n", t2 - t1);

	t1 = timestamp();
	const struct linop_s* Fx = linop_fx_create(wx, sy, sz, nc, ne);
	t2 = timestamp();
	debug_printf(DP_INFO, "\tFx:  %f seconds.\n", t2 - t1);

	t1 = timestamp();
	const struct linop_s* W = linop_wave_create(wx, sy, sz, nc, ne, wave);
	t2 = timestamp();
	debug_printf(DP_INFO, "\tW:   %f seconds.\n", t2 - t1);

	t1 = timestamp();
	const struct linop_s* Fyz = linop_fyz_create(wx, sy, sz, nc, ne);
	t2 = timestamp();
	debug_printf(DP_INFO, "\tFyz: %f seconds.\n", t2 - t1);

	t1 = timestamp();
	const struct linop_s* M   = linop_samp_create(wx, sy, sz, nc, ne, mask);
	t2 = timestamp();
